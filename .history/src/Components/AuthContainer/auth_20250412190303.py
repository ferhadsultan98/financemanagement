...
cred = credentials.Certificate("src/Components/AuthContainer/config/finance.json")
initialize_app(cred)
db = firestore.client()
...
def verify_otp():
    ...
    try:
        ...
        # Firestore Timestamp'i offset-naive datetime'a çevir
        expires_at_naive = expires_at.to_datetime().replace(tzinfo=None)
        ...
        # OTP doğrulandı, kaydı sil ve başarılı yanıt dön
        otp_ref.delete()
