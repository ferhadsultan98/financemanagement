...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      console.log("Giriş denemesi:", { username, password }); // Girilen bilgileri logla
      const credentialsRef = collection(db, "credentials");
      const querySnapshot = await getDocs(credentialsRef);

      let userFound = false;
      console.log("Firestore'dan çekilen belgeler:", querySnapshot.docs.length); // Belge sayısını logla

      querySnapshot.forEach((doc) => {
        const data = doc.data();
        console.log("Belge verisi:", data); // Her belgeyi logla
