...
const Sidebar = () => {
  ...
  const handleTopUp = async () => {
    if (!topUpAmount || parseFloat(topUpAmount) <= 0) {
      toast.error(t("sidebar.invalidamountmessage"), { position: "top-right", autoClose: 3000, style: { top: "30px" }  });
      return;
    }
    ...
    setTopUpAmount("");
    toast.success(`${amount} ₼ ${t("sidebar.moneysuccessmessage")}`, { position: "top-right", autoClose: 2000, style: { top: "30px" }  });
  };

  const handleLogout = () => {
    localStorage.removeItem("isLoggedIn");
    localStorage.removeItem("username");
    toast.info(t("sidebar.logoutmessage"), { position: "top-right", autoClose: 2000, style: { top: "30px" }  });
    navigate("/login");
  };
