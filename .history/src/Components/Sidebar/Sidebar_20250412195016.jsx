...
const Sidebar = () => {
  ...
  const [userProfile, setUserProfile] = useState(null); // Varsayılan değerler kaldırıldı
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        setIsLoading(true); // Veri çekme başlamadan loading true
        ...
        if (snapshot.exists()) {
          ...
          await set(userRef, { profile: null, balance: 0 });
          setBalance(0);
        }
      } catch (err) {
        ...
        setIsLoading(false); // Veri çekme bittiğinde loading false
      }
    };
    ...
    const unsubscribe = onValue(userRef, (snapshot) => {
      ...
      setIsLoading(false); // Realtime güncelleme sonrası loading false
    });
  ...
  return (
    <div className="sidebar-container">
      ...
      {isModalOpen && (
        <ProfileModal
          userProfile={userProfile || {}} // userProfile null ise boş obje geç
