...
const Sidebar = () => {
  ...
  // Realtime Database'den profil ve balans bilgisini fetch et
  useEffect(() => {
    if (!username) {
    ...
    // İlk yüklemede profil ve balans bilgisini al
    const fetchInitialData = async () => {
      try {
        ...
        console.error("Realtime Database fetch error:", err);
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          const data = snapshot.val();
          setBalance(data.balance || 0);
          setUserProfile(data.profile || userProfile); // Profil de güncellenir
        }
      },
      (err) => {
        console.error("Realtime Database snapshot error:", err);
  ...
  const handleProfileUpdate = async (updatedProfile) => {
    ...
    try {
      ...
      console.error("Realtime Database profile update error:", err);
