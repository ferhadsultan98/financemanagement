...
const Sidebar = () => {
  ...
  const handleTopUp = async () => {
    if (!topUpAmount || parseFloat(topUpAmount) <= 0) {
      toast.error(t("sidebar.invalidamountmessage"), { position: "top-right", autoClose: 3000 });
      return;
    }
    ...
    setTopUpAmount("");
    toast.success(`${amount} ₼ ${t("sidebar.moneysuccessmessage")}`, { position: "top-right", autoClose: 3000 });
  };

  const handleLogout = () => {
    localStorage.removeItem("isLoggedIn");
    localStorage.removeItem("username");
    toast.info(t("sidebar.logoutmessage"), { position: "top-right", autoClose: 2000 });
    navigate("/login");
  };
  ...
  const handleProfileUpdate = async (updatedProfile) => {
    ...
    toast.success(t("sidebar.updatemessage"), { position: "top-right", autoClose: 3000 });
  };
