...
import { db, doc, getDoc, setDoc, onSnapshot } from "../../Backend/Firebase";
...
const Sidebar = () => {
  ...
  // Firestore'dan profil və balans məlumatlarını dinamik çək
  useEffect(() => {
    if (!username) {
      navigate("/login");
      return;
    ...
    // İlk balans və profil məlumatlarını çək
    const fetchInitialData = async () => {
      ...
      if (docSnap.exists()) {
        ...
        await setDoc(userDocRef, { profile: userProfile, balance: 0 }, { merge: true });
        setBalance(0);
      }
    };
    ...
    // Balansın real vaxtda yenilənməsi üçün dinləyici
  ...
  const handleTopUp = async () => {
    ...
    const userDocRef = doc(db, "users", username);
    await setDoc(userDocRef, { profile: userProfile, balance: newBalance }, { merge: true });
