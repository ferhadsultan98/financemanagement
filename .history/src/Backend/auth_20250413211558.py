...
# CORS'u tüm origin'ler için açıkça etkinleştiriyoruz
CORS(app, resources={r"/*": {"origins": "*"}}, supports_credentials=False)

# Tüm isteklerden sonra CORS başlıklarını ekliyoruz
@app.after_request
...
# OPTIONS isteklerini açıkça ele alıyoruz
@app.route('/send-otp', methods=['OPTIONS'])
@app.route('/verify-otp', methods=['OPTIONS'])
@app.route('/send-welcome', methods=['OPTIONS'])
def handle_options():
    return '', 200
