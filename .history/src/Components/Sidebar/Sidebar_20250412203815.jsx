...
import { realtimeDb, ref, get, set, onValue } from "../../Backend/realtime";
...
const Sidebar = () => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          await set(userRef, { profile: null, balance: 0, username: "" });
          setBalance(0);
        }
      } catch (err) {
  ...
  const handleTopUp = async () => {
    ...
    try {
      const userRef = ref(realtimeDb, `users/${userId}`);
      await set(userRef, { profile: userProfile, balance: newBalance, username: userProfile?.username || "" });
      setTopUpAmount("");
  ...
  const handleProfileUpdate = async (updatedProfile) => {
    ...
    try {
      const userRef = ref(realtimeDb, `users/${userId}`);
      await set(userRef, { profile: updatedProfile, balance, username: userProfile?.username || "" });
