...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      // Firestore'dan credentials koleksiyonunu fetch et
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (data.username === username) {
          ...
          if (data.password !== password) {
            userFound = false; // Şifre yanlışsa giriş başarısız
          }
        }
      });
      ...
      if (userFound) {
        ...
        navigate("/");
      } else if (!userFound && !userDocId) {
        // Kullanıcı yoksa ekle (isteğe bağlı, kaldırılabilir)
        const newUserRef = doc(db, "credentials", `user-${Date.now()}`);
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      // Firestore'da username'e göre şifreyi güncelle
      const credentialsRef = collection(db, "credentials");
      const querySnapshot = await getDocs(credentialsRef);
      let userDocId = null;
      ...
      if (userDocId) {
        const userDocRef = doc(db, "credentials", userDoc ducts:
      ...
      <div className="loginWrapper">
        ...
        <div className="login-right">
          ...
          <div className="login-box">
            ...
            {resetMessage && (
              <div className={resetMessage === t("login.reset.success") ? "success-message" : "error-message"}>
                {resetMessage}
              </div>
            )}
