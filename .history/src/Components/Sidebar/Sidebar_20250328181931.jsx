...
import { db, doc, getDoc, setDoc, onSnapshot } from "../../Backend/Firebase"; // Firestore imports
...
const Sidebar = () => {
  ...
  // Fetch and listen to balance and profile from Firestore
  useEffect(() => {
    if (!username) {
    ...
    const userDocRef = doc(db, "users", "user-id"); // Use the same user-id as in AddExpense
    ...
    const unsubscribe = onSnapshot(userDocRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setUserProfile(data.profile || userProfile); // Update profile if stored in Firestore
      } else {
        // If document doesn't exist, initialize it
        setDoc(userDocRef, { profile: userProfile, balance: 0 }, { merge: true });
        setBalance(0);
      }
    }, (err) => {
  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      toast.error("Balans güncelleme hatası: " + err.message, {
        position: "top-right",
        autoClose: 3000,
        style: { top: "30px" },
      });
  ...
  const handleProfileUpdate = async (updatedProfile) => {
    ...
    try {
      ...
      toast.error("Profil güncelleme hatası: " + err.message, {
        position: "top-right",
        autoClose: 3000,
        style: { top: "30px" },
      });
