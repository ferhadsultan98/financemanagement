...
const Sidebar = () => {
  ...
  const [balance, setBalance] = useState(0); // Balansı yerel state olarak tanımla
  const navigate = useNavigate();
  const username = localStorage.getItem("username");

  // Profil ve balans bilgisini Firestore'dan fetch et
  useEffect(() => {
    if (username) {
      const userDocRef = doc(db, "users", username);

      // Profil bilgisini fetch et
      const fetchProfile = async () => {
        const docSnap = await getDoc(userDocRef);
        if (docSnap.exists()) {
          setUserProfile(docSnap.data().profile || userProfile);
          setBalance(docSnap.data().balance || 0); // İlk balans değerini al
        } else {
          await setDoc(
            userDocRef,
  ...
  const handleTopUp = async () => {
    ...
    if (username) {
      ...
      setBalance(newBalance); // Yerel durumu güncelle
      setTopUpAmount("");
